	...
	mJumpToSelected(false), mMetadataChanged(false)
{
	...
	if (!mFromPlaceholder) {
		...
		if (currentSort == reqSort) {
			...
			mJumpToLetterList = std::make_shared<LetterList>(mWindow, Locale::getInstance()->gettext("jumpTo").c_str(), false);
			...
			row.addElement(std::make_shared<TextComponent>(mWindow, Locale::getInstance()->gettext("jumpTo").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		...
		if (!rpConfigSelected && useGamelistMedia && (!collectionsSelected || collectionsSelected && file->getType() == GAME)) {
			row.elements.clear();
			row.addElement(std::make_shared<TextComponent>(mWindow, Locale::getInstance()->gettext("launchSystemScreensaver").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		...
		mMenu.addWithLabel(Locale::getInstance()->gettext("sortGamesBy"), mListSort);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, Locale::getInstance()->gettext("filterGamelist").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, Locale::getInstance()->gettext("addRemoveGamesToThisGameCollection"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, Locale::getInstance()->gettext("finishEditing") + " '"+ Utils::String::toUpper(CollectionSystemManager::get()->getEditingCollection()) + "' COLLECTION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, Locale::getInstance()->gettext("getNewRandomGames"), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
...
std::vector<HelpPrompt> GuiGamelistOptions::getHelpPrompts()
{
	...
	prompts.push_back(HelpPrompt("b", Locale::getInstance()->gettext("close")));
	return prompts;
}
